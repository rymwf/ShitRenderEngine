...
const char *skyboxImageName = "images/sunset_jhbcentral_4k.hdr";
...
class Hello : public App
{
	...
	void createScene()
	{
		...
		for (uint32_t i = 1; i < testModelResources.size(); ++i)
		{
			testModels.emplace_back(scene->createModel(testModelResources[i]));
		}

		testModels[1]->translate({0, 1, 0})->scale({0.5, 0.5, 0.5});
		testModels[2]->translate({-2, 0, 0});
	...
	void showInspectorSceneNode(SceneNode *sceneNode)
	{
		...
		if (ImGui::InputFloat3("position", (float *)&translation))
		{
